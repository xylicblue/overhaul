...
const PnLChart = ({ canonicalEvents = [] }) => {
  ...
  const chartColor   = isProfitable ? "#29d28b" : "#f5484e";
  ...
  if (pnlEvents.length === 0) {
    return (
      <div className="bg-surface-1 border border-line-subtle rounded-xl p-6 backdrop-blur-sm">
        <h3 className="text-sm font-semibold text-ink mb-4">P&L Performance</h3>
  ...
  return (
    <div className="bg-surface-1 border border-line-subtle rounded-xl p-5 backdrop-blur-sm">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h3 className="text-sm font-semibold text-ink">P&L Performance</h3>
          <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${
            isProfitable ? "bg-up/10 text-up" : "bg-down/10 text-down"
          }`}>
        ...
        <div className="flex gap-1 bg-surface-2/40 rounded-lg p-0.5">
          {["24h", "7d", "30d", "all"].map((range) => (
            <button
              key={range}
              onClick={() => setTimeRange(range)}
              className={`px-2.5 py-1 text-[10px] font-bold rounded-md transition-all ${
                timeRange === range ? "bg-surface-3 text-ink" : "text-ink-faint hover:text-ink-muted"
              }`}
            >
              {range.toUpperCase()}
            </button>
