    ...
    return (
      <div className="flex flex-col h-full bg-slate-950/50 backdrop-blur-sm">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-slate-500 p-6 text-center">
          ...
          <p className="text-slate-300 font-medium mb-1">Wallet Not Connected</p>
          <span className="text-xs text-slate-500 max-w-[200px]">Connect your wallet to view your active positions and performance.</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col h-full bg-slate-950/50 backdrop-blur-sm">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-slate-500">
          ...
          <span className="text-xs font-medium animate-pulse">Loading positions...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col h-full bg-slate-950/50 backdrop-blur-sm">
        <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between sticky top-0 bg-slate-950/80 backdrop-blur-md z-10">
          ...
          <span className="bg-slate-800 text-slate-400 text-[10px] font-bold px-2 py-0.5 rounded-full">0</span>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center text-slate-500 p-6 text-center">
          ...
          <span className="text-xs text-slate-500 max-w-[200px]">Open a position in the market to see it tracked here.</span>
        </div>
      </div>
    );
  }
...
function PositionCard({
  ...
  return (
    ...
    >
      {/* Background Glow */}
      <div className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-[60px] opacity-20 pointer-events-none ${isLong ? "bg-green-500" : "bg-red-500"}`}></div>

      <div className="p-4 relative z-10">
        {/* Header */}
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-lg ${isLong ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400"}`}>
              {isLong ? <TrendingUp size={18} /> : <TrendingDown size={18} />}
            </div>
            <div>
              <div className="font-bold text-white text-sm tracking-wide">{position.marketName}</div>
              <div className="flex items-center gap-2 mt-0.5">
                <span className={`text-[10px] font-bold uppercase tracking-wider ${isLong ? "text-green-400" : "text-red-400"}`}>
                  {isLong ? "Long" : "Short"}
                </span>
                <span className="text-[10px] text-slate-500 font-mono">|</span>
                <span className="text-[10px] text-slate-300 font-mono">{absSize.toFixed(4)} Size</span>
              </div>
            </div>
          </div>
          
          <div className="text-right">
            <div className={`font-mono font-bold text-base ${isProfitable ? "text-green-400 drop-shadow-[0_0_8px_rgba(34,197,94,0.3)]" : "text-red-400 drop-shadow-[0_0_8px_rgba(239,68,68,0.3)]"}`}>
        ...
        <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs mb-4 bg-slate-950/30 rounded-lg p-3 border border-white/5">
          <div className="flex justify-between items-center">
            ...
            <span className="text-slate-200 font-mono">${entryPrice.toFixed(2)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-slate-500">Mark Price</span>
            <span className="text-slate-200 font-mono">${currentPrice > 0 ? currentPrice.toFixed(2) : "..."}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-slate-500">Notional</span>
            <span className="text-slate-200 font-mono">${openNotional.toFixed(2)}</span>
          </div>
          <div className="flex justify-between items-center">
